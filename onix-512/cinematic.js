    ...
    docGrid.innerHTML = DOCS.map(doc => `
      <article class="doc-card reveal">
        ...
        <div class="doc-actions">
          <a href="${pdfUrl(doc.key)}" target="_blank" rel="noopener">Abrir PDF ↗</a>
          <a href="${downloadUrl(doc.key)}" target="_blank" rel="noopener" data-download>Descargar ↓</a>
        </div>
      </article>`).join('');
  }
  ...
  if (planGrid) {
    planGrid.innerHTML = PLANS.map(plan => `
      <article class="plan-card reveal">
        <img data-asset="${plan.asset}" alt="Vista previa: ${plan.title}" loading="lazy">
        <div>
          <small>${plan.no}</small>
          <h3>${plan.title}</h3>
          <div class="plan-actions">
            <a href="${pdfUrl(plan.key)}" target="_blank" rel="noopener">Abrir PDF ↗</a>
            <a href="${downloadUrl(plan.key)}" target="_blank" rel="noopener" data-download>Descargar ↓</a>
          </div>
        </div>
      </article>`).join('');
  }
...
function initDownloads() {
  $$('[data-download]').forEach(link => link.addEventListener('click', () => {
    showToast('Preparando la descarga del PDF oficial.');
  }));
}
